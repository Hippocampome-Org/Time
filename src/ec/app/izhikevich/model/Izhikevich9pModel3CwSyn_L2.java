 ...
 * ((0))====[1]====[2]
 */
public class Izhikevich9pModel3CwSyn_L2 extends Izhikevich9pModelMCwSyn{
		
	public Izhikevich9pModel3CwSyn_L2(int nComps) {
		super(nComps);
	}
	...
	public void computeDerivatives(double t, double[] y, double[] dy)
		...
		//Dendrite2
		double iFromDend1 = g[1] * (1-p[1])* (V1 - V2);	
		dy[5] = ((k[2] * (V2 - vR[2]) * (V2 - vT[2]))  - U2 + iFromDend1 + appCurrentDend2 + iSynapse2) / cM[2];
		dy[6] = a[2] * ((b[2] * (V2 - vR[2])) - U2);
				
		//Dendrite1
		double iFromSoma = g[0] * (1-p[0])*(V0 - V1);
		double iFromDend2 = g[1] * (p[1])* (V2 - V1);		
		dy[2] = ((k[1] * (V1 - vR[1]) * (V1 - vT[1]))  - U1 + iFromSoma +  iFromDend2 + appCurrentDend1 + iSynapse1) / cM[1];
		dy[3] = a[1] * ((b[1] * (V1 - vR[1])) - U1);	
		
		// Soma	
		iFromDend1 = g[0] * (p[0])* (V1 - V0);		
		dy[0] = ((k[SOMA_IDX] * (V0 - vR[SOMA_IDX]) * (V0 - vT[SOMA_IDX]))  - U0 + iFromDend1 + appCurrentSoma) / cM[SOMA_IDX];
